...
\end{lstlisting}
	...
	Le type de retour de Wheat.process() et de Mill.process() sont différents (problème de type de retour covariant (cf ligne 157 de Notes.txt)) donc on doit modifier le type de retour d'une des deux méthodes, donc ici on devrait mettre dans la classe WheatMill : Grain process${$ ... $}$
	\newpage
\begin{lstlisting}
class Egg2 {
...
$ Egg2.Yolk.f()
\end{lstlisting}
	\noindent Ne pas oublier que on appelle d'abord le constructeur des parents de la classe courante avant d'appeler le constructeur de la classe courante ; On a une variable qui appelle un constructeur dans la classe (à revoir parce que changer la ligne de la définition de la variable ne change rien : pourquoi ?)
	\newpage
...
\end{lstlisting}
	...
	Deux solutions :
	\begin{itemize}
		\item mettre PContents en statique puis Parcel4.PContents pc = new Parcel4.PContents();
		\item créer un objet Contents à partir de la méthode contents() puis cast l'objet en Parcel4.PContents
	\end{itemize}
	\newpage
...
\end{lstlisting}
	...
	\item Given interface FooBar<X,Y> extends Silly<X> which of the following ARE subtypes of Silly<String>? b and d (X has to be String and Y has to be not String)
	\begin{enumerate}
	...
	\end{quote}
	\noindent There is an error because the two arguments of pick() method have to be the same type but Set<> and Stack<> aren't the same type.
	\newpage
...
\end{lstlisting}
	\noindent The code above doesn't give an error because an ArrayList is a List and ? super Integer includes Number.
	\item If the code below gives an error explain why, if not explain why.
\begin{lstlisting}
...
\end{lstlisting}
\noindent There is no compile-time error because we only deal with Number objects.
\begin{lstlisting}
class Example{
...
\end{lstlisting}
	...
	The error is because we try to modify the value of Out.x with the Out.In.SetX() method. Why?
...
\begin{lstlisting}
// Reponse question 19 (code au dessus)
$ kitten
$ cat
$ Lion
\end{lstlisting}
...
\begin{lstlisting}
// Reponse question 20 (code au dessus)
$ cat
$ Lion
$ young Lion
// Faux : on ne peut pas mettre un Lion.Kitten dans un Cat.Kitten
\end{lstlisting}
\noindent To fix it, we have to make the Cat.Kitten class static, then we create a new Cat.Kitten() in Lion constructor, then the output is cat, Lion, kitten
\begin{lstlisting}
class A {
 	private int x;
...
\end{lstlisting}
	...
	The race condition exist on the if(y\textless 5) because we modify the value of y. If two threads access at the same time to the y value and y==4, y will be incremented by the number of current threads on it. So, at the end of the calls of foo, y might be greater or equal to 5.
\begin{lstlisting}
