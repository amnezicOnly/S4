...
\end{lstlisting}
	...
	The race condition is on line 10 because multiple threads try to access to data and Race.MAX at the same time and try to increment it at the same time. Example : if data = 4, two threads may add one to data at the sale time then data = 6 but shouldn't be greater than 5.
	...
	A deadlock is a situation where a thread threadA wait a other thread threadB to give a signal, but theadB wait for threadA to give a signal (endless loop).
\newpage
\begin{lstlisting}
class Examine{
...
\end{lstlisting}
	...
	No error with recent versions of Java.
	\newpage
\begin{lstlisting}
class Egg2 {
